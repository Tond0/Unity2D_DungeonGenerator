...
public class Generator_BPSTree : Generator
{
    [SerializeField] private Vector2 minRatio = new(.45f,.45f);
    [SerializeField] private int minSectionSize = 1;

    [Header("Debug")]
    [SerializeField] private List<Rect> rooms = new();

    DungeonSection firstSection;

    public override void Generate(Vector2 firstSectionCenter)
    {
        rooms.Clear();
        tilemap.ClearAllTiles();

        Rect originalSection = new(firstSectionCenter.x, firstSectionCenter.y, size.x, size.y);
    ...
    protected virtual void DrawRoom(DungeonSection sectionToCreateRoomIn)
    {
        //Create room rect
        Rect roomToCreate = new();

        roomToCreate.x = sectionToCreateRoomIn.Rect.x + Mathf.FloorToInt(UnityEngine.Random.Range(1, sectionToCreateRoomIn.Rect.width / 3));
        roomToCreate.y = sectionToCreateRoomIn.Rect.y + Mathf.FloorToInt(UnityEngine.Random.Range(1, sectionToCreateRoomIn.Rect.height / 3));

        roomToCreate.width = sectionToCreateRoomIn.Rect.width - (roomToCreate.x - sectionToCreateRoomIn.Rect.x);
        roomToCreate.height = sectionToCreateRoomIn.Rect.height - (roomToCreate.y - sectionToCreateRoomIn.Rect.y);
        roomToCreate.width -= Mathf.FloorToInt(UnityEngine.Random.Range(1, roomToCreate.width / 3));
        roomToCreate.height -= Mathf.FloorToInt(UnityEngine.Random.Range(1, roomToCreate.height / 3));

        //Draw room rect
        Vector2Int bottomLeftCorner = Vector2Int.FloorToInt(roomToCreate.position);

        // Loop per disegnare il rettangolo
        for (int i = 0; i < roomToCreate.width; i++)
        {
            for (int j = 0; j < roomToCreate.height; j++)
            {
                Vector2Int offset = new Vector2Int(i, j);
                if (i == 0 || i == roomToCreate.width - 1
                    || j == 0 || j == roomToCreate.height - 1)
    ...
    private void OnDrawGizmoSelected()
    {
        ...
        Vector3 cubeSize = new(firstSection.Rect.width, firstSection.Rect.height, 0);
        Gizmos.DrawWireCube(firstSection.Rect.center, cubeSize);
    }
